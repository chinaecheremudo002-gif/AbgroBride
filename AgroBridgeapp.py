...
# Previous price (last available price)
lag_1 = df_filtered["PRICE"].iloc[-1] if len(df_filtered) >= 1 else 0

# Price 3 periods ago
lag_3 = df_filtered["PRICE"].iloc[-3] if len(df_filtered) >= 3 else lag_1

# -------- Rolling Average --------

# Average of the last 3 prices
rolling_avg = (
    df_filtered["PRICE"]
...
# Latest minimum distance
min_dist = (
    df_filtered["MIN_DIST_S"].iloc[-1]
    if len(df_filtered) >= 1
    else 0
)

# Latest mean distance
mean_dist = (
    df_filtered["MEAN_DIST_S"].iloc[-1]
    if len(df_filtered) >= 1
    else 0
)

# for price prediction _____________________________________________________
...
# Previous price (last available price)
lag_12 = df_filtered2["PRICE"].iloc[-1] if len(df_filtered2) >= 1 else 0

# Price 3 periods ago
lag_32 = df_filtered2["PRICE"].iloc[-3] if len(df_filtered2) >= 3 else lag_1
...
# Latest minimum distance
min_dist2 = (
    df_filtered2["MIN_DIST_S"].iloc[-1]
    if len(df_filtered2) >= 1
    else 0
)

# Latest mean distance
mean_dist2 = (
    df_filtered2["MEAN_DIST_S"].iloc[-1]
    if len(df_filtered2) >= 1
    else 0
)
# ----------------------------
