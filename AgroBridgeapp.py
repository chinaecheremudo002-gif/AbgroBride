...
if st.button("🚀 Predict Price Signal"):
    ...
    with st.spinner("Analyzing market data... please wait"):
        ...
        if  Analysis_type == "📈 Price Prediction Insight":
            ...
            col4.metric( "⚙️ Predicted Next Price",f"{float(predict2):.2f}")
            ...
            if label == "Rising":
                st.success("Hold on,prices are rising, wait before selling.")
            elif label in ["Falling", "Failing", "Down"]:
                st.error("Consider selling now before prices drop further")
            else:
